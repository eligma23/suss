...
async def neural_rerank(query: str, chunks: list[Chunk]) -> list[Chunk]:
    chunks = [chunk.to_string() for chunk in chunks[:COHERE_MAX_CHUNKS]]
    response = await client.rerank(
        model="rerank-multilingual-v3.0", query=query, documents=chunks
    )
...
async def rank_chunks(query: str, chunks: list[Chunk]) -> list[Chunk]:
    if os.getenv("COHERE_API_KEY", None):
        return neural_rerank(query, chunks)
...
class CodeSearchTool(Tool):
    ...
    async def run(self, intent: str, query: str, **kwargs) -> List[Chunk]:
        ...
        results = await rank_chunks(query, results)
